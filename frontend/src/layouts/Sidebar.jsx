...
const menuSections = [
  ...
  },
  {
    category: "ออเดอร์",
    collapsible: false,
    items: [{ title: "ออเดอร์", path: "/order", icon: BarChart3, roles: [1, 2] }],
  },
  {
    category: "การจัดส่ง",
    collapsible: false,
    items: [{ title: "รายการการจัดส่ง", path: "/delivery", icon: Truck, roles: [1, 2] }],
  },
  {
    category: "บิล / การค้างถัง",
    collapsible: true,
    items: [
      { title: "บิลรายการออเดอร์", path: "/bills/orders", icon: Receipt, roles: [1] },
  ...
  },
  {
    ...
    collapsible: true,
    items: [
      { title: "ข้อมูลพนักงาน", path: "/employee", icon: UserCheck, roles: [1] }, // ให้เฉพาะเจ้าของ (1) เห็น
  ...
  },
  {
    ...
    collapsible: false,
    items: [
      { title: "จัดการรหัสผ่าน", path: "/security", icon: Lock, roles: [1] }, // ให้เฉพาะเจ้าของ (1) เห็น
    ],
  }
];
...
export default function Sidebar() {
  ...
  // แปลง role เป็นตัวเลขเพื่อความปลอดภัยในการเปรียบเทียบ (รองรับกรณีที่ API ส่งมาเป็น string)
  ...
  const [openSections, setOpenSections] = useState(
    Object.fromEntries(
      filteredMenuSections.filter((s) => s.collapsible).map((s) => [s.category, true]),
    ),
  );
