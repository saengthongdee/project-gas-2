...
import { FolderOpen, Plus, Trash2, AlertCircle, Loader2 } from 'lucide-react';
import { useMaintenance } from '../hook/useMaintenance'; 
import MaintenanceSlideOver from '../MaintenanceSlideOver'; 

export default function Maintenance() {
  const { data: maintenances, loading, error, createMaintenance, deleteMaintenance, refetch } = useMaintenance();
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="flex items-start gap-3.5">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold text-[#1A1A1A]">แฟ้มประวัติการซ่อมบำรุง</h1>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-neutral-100 text-neutral-600 border border-neutral-200">
                {Array.isArray(maintenances) ? maintenances.length : 0} รายการ
              </span>
            </div>
            <p className="text-xs sm:text-sm text-neutral-500 mt-0.5">จัดการและเรียกดูรูปภาพประวัติการซ่อมบำรุงทั้งหมด</p>
          </div>
        </div>

        <button
          onClick={() => setIsSlideOverOpen(true)}
          className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#3a66cc] hover:bg-[#2d52a8] text-white text-sm font-semibold rounded-xl transition-colors shadow-sm cursor-pointer shrink-0"
        >
          <Plus className="w-4 h-4" />
          <span>บันทึกรายการซ่อมใหม่</span>
        </button>
      </div>
      ...
      {loading && (!maintenances || maintenances.length === 0) ? (
          ...
          {maintenances.map((item) => (
            ...
            >
              ...
              <div 
                ...
                onClick={() => item.imageUrl && window.open(`http://localhost:5000/${item.imageUrl}`, '_blank')}
        ...
        <div className="flex flex-col items-center justify-center py-16 sm:py-20 bg-white rounded-2xl border border-neutral-200 shadow-sm px-4 text-center">
          ...
          <p className="text-base font-bold text-neutral-700">ยังไม่มีประวัติการซ่อมบำรุง</p>
          <p className="text-xs text-neutral-400 mt-1">กดปุ่ม "บันทึกรายการซ่อมใหม่" ด้านบนเพื่อเริ่มเก็บประวัติ</p>
        </div>
      )}
